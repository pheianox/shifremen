...
const ConfirmExportPasswordModal: Modal<string | null, Props> = props => {
  ...
  async function match() {
    ...
    if (await props.onMatch(password_)) {
      ...
      setError(translate("modals.cepm.error"))
    }
  }
  ...
  return (
    <div class="modal modal-open modal-bottom sm:modal-middle z-50 " onclick={cancel}>
      <div class="relative bg-base-100 w-[400px] p-8 rounded-lg flex flex-col gap-4" onclick={evt => evt.stopPropagation()}>
        <h1 class="text-xl font-bold">{translate("modals.cepm.title")}</h1>
        <div class="flex flex-col gap-3">
          <p>{translate("modals.cepm.descr")}</p>
        </div>
...
export default ConfirmExportPasswordModal
